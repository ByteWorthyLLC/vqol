...
| GitHub repository social-preview upload | Generated asset exists, but GitHub CLI has no social-preview upload flag and this browser session lacks repository settings access |
| Physical iOS Safari install/persistence | Requires actual iOS hardware or an authenticated device-cloud session |
| Physical Android Chrome install/notification | Requires actual Android hardware or an authenticated device-cloud session |
| Native print/save dialogs | Headless automation verifies `window.print()` invocation, but not OS-native dialogs |
| VoiceOver/NVDA manual audit | Requires assistive-technology runtime and human navigation |
...
Physical device and screen-reader checks should be recorded before announcing a
patient-facing public release.
